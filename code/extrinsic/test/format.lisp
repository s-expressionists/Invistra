...
(defmacro define-char-test (name func control-string)
  `(define-test ,name
     (my-with-standard-io-syntax
      ,@(loop for char across +standard-chars+
              collect `(is-equal ,(funcall (eval func) char)
                                 (fmt nil ,control-string ,char))))))

(define-char-test character.01
  #'string
  "~c")

(define-char-test character.02
    (lambda (char)
...
(define-equal-test aesthetic.01
  "234"
  (fmt nil "~a" 234))

(define-equal-test aesthetic.02
  "-10"
  (fmt nil "~a" -10))

(define-equal-test aesthetic.03
  "1.5"
  (fmt nil "~a" 1.5))

(define-equal-test aesthetic.04
  "ABC"
  (fmt nil "~a" 'abc))

(define-equal-test aesthetic.05
  "hello"
  (fmt nil "~a" "hello"))

(define-equal-test aesthetic.06
  "x"
  (fmt nil "~a" #\x))

(define-equal-test aesthetic.07
  " "
  (fmt nil "~a" #\Space))

(define-equal-test aesthetic.08
  "NIL"
  (fmt nil "~a" nil))

;; Test that with a `:' modifier, NIL will print as ()
(define-equal-test aesthetic.09
  "()"
  (fmt nil "~:a" nil))

;; Test that the mincol argument is taken into account
(define-equal-test aesthetic.10
  "hello  "
  (fmt nil "~7a" "hello"))
...
(define-equal-test aesthetic.11
  "hello                     "
  (fmt nil "~21,7a" "hello"))

;; Test that the minpad parameter is taken into account
(define-equal-test aesthetic.12
  "hello   "
  (fmt nil "~,,3a" "hello"))

;; Test that the padchar parameter is taken into account
(define-equal-test aesthetic.13
  "helloxxx"
...
(define-equal-test standard.01
  "234"
  (fmt nil "~s" 234))

(define-equal-test standard.02
  "-10"
  (fmt nil "~s" -10))

(define-equal-test standard.03
  "1.5"
  (fmt nil "~s" 1.5))

(define-equal-test standard.04
  "ABC"
  (fmt nil "~s" 'abc))

(define-equal-test standard.05
  "\"hello\""
  (fmt nil "~s" "hello"))
...
(define-equal-test standard.07
  "#\\Space"
  (fmt nil "~s" #\Space))

(define-equal-test standard.08
  "NIL"
  (fmt nil "~s" nil))

;; Test that with a `:' modifier, NIL will print as ()
(define-equal-test standard.09
  "()"
  (fmt nil "~:s" nil))

;; Test that the mincol argument is taken into account
(define-equal-test standard.10
  "12345  "
  (fmt nil "~7s" 12345))
...
;; Test that the colinc parameter is taken into account
(define-equal-test standard.11
  "12345                     "
  (fmt nil "~21,7s" 12345))

;; Test that the minpad parameter is taken into account
(define-equal-test standard.12
  "12345   "
  (fmt nil "~,,3s" 12345))

;; Test that the padchar parameter is taken into account
(define-equal-test standard.13
  "12345xxx"
